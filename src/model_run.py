...
class RWKV_RNN(MyModule):
    def __init__(self, args):
        ...
        elif args.strategy == "fp16":
            self.FLOAT_MODE = torch.half
        elif args.strategy == "bf16":
            self.FLOAT_MODE = torch.bfloat16

        with torch.no_grad():
            w = torch.load(args.model + ".pth", map_location="cpu")
            print(f"{w=}")
            gc.collect()
    ...
    def forward(self, tokens, state, preprocess_only=False):
        with torch.no_grad():
            ...
            if state == None:
                state = torch.zeros(args.n_layer * 5, args.n_embd, device=args.device)
...
def get_args():
    p = argparse.ArgumentParser(prog="model_run", description="simple script to run rwkv")
    p.add_argument(
        "--model",
        metavar="MODEL_NAME",
        help="model path",
        default="/data/BlinkDL/RWKV-4-Pile-14B-20230313-ctx8192-test1050",
    )
    p.add_argument(
        "--strategy",
        metavar="FLOAT_MODE",
    ...
    p.add_argument("--device", metavar="RUN_DEVICE", help="Device to run on.", default="cuda:0")
    return p.parse_args()
